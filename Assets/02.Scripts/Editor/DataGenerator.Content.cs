...
{
    /// <summary>
    /// DataGenerator — 콘텐츠 데이터 생성 (아이템, 이벤트, UIPalette, AudioPalette)
    ...
    public static partial class DataGenerator
    {
            ...
            ItemType type, ItemEffectType effectType, int effectValue, int price, RewardRarity rarity)
        {
            ...
            // 아이콘 자동 할당
            string iconPath = GetItemIconPath(effectType);
            var icon = AssetDatabase.LoadAssetAtPath<Sprite>(iconPath);
            if (icon != null)
                SetPrivateField(item, "_icon", icon);
            else
                Debug.LogWarning($"[DataGenerator] 아이템 아이콘 없음: {iconPath} ({fileName})");

            EditorUtility.SetDirty(item);
        }

        /// <summary>
        /// ItemEffectType → Layer Lab ItemIcons 아이콘 경로 매핑
        /// </summary>
        private static string GetItemIconPath(ItemEffectType effectType)
        {
            return effectType switch
            {
                ItemEffectType.MaxHPUp         => $"{ITEM_BASE}/Icon_Heart.png",
                ItemEffectType.ATKUp           => $"{ITEM_BASE}/Icon_Sword.png",
                ItemEffectType.DEFUp           => $"{ITEM_BASE}/Icon_Shield.png",
                ItemEffectType.HealPerTurn     => $"{ITEM_BASE}/Icon_Potion01_Red.png",
                ItemEffectType.ExtraGold       => $"{ITEM_BASE}/Icon_Gold.png",
                ItemEffectType.DrawWeightBonus => $"{ITEM_BASE}/Icon_Star.png",
                _                              => $"{ITEM_BASE}/Icon_Bag.png"
            };
        }
