...
23. **★ VerticalLayoutGroup이 LayoutElement.preferredHeight를 무시** (2026-07-19): VLG는 자체적으로 ILayoutElement라 자식 LayoutElement보다 우선할 때가 있음. 고정 크기 헤더/푸터는 VLG 대신 **anchor 직접 배치** (`anchorMin/Max/pivot/sizeDelta` 또는 `offsetMin/Max`) 사용할 것.
24. **★ GetComponentInChildren는 자손만 검색** (2026-07-19): 형제 컴포넌트는 못 찾음. Builder가 컨테이너별로 컴포넌트를分散 부착하면 AutoBindMissingFields가 실패. 해결: (a) `transform.parent.GetComponentInChildren<T>(true)` 또는 (b) 명시적 WireField로 연결 또는 (c) 단일 부모 아래 자식-손자 구조로 재배치.
25. **★ SerializeField 인스펙터 값이 코드 기본값보다 우선** (2026-07-19): `[SerializeField] private float _x = 110f;` 코드를 바꿔도 씬에 저장된 값이 우선. Builder가 명시적으로 `SerializedObject.FindProperty + floatValue =` 설정하지 않으면 코드 수정 효과 없음.
26. **★ RectTransform.rect.height가 Initialize 시점에 0** (2026-07-19): Canvas 레이아웃 평가 전에 Initialize가 호출되면 rect.height=0. 3중 폴백 필수: (1) `LayoutRebuilder.ForceRebuildLayoutImmediate` (2) 조상 트리 순회 (3) 절대값 (예: 900f).
27. **★ Awake에서 enabled=false 금지** (2026-07-19): MonoBehaviour는 enabled=false 시 Start()를 호출하지 않음. Update 1프레임 뒤 체크 또는 IEnumerator Start + WaitForEndOfFrame 패턴 사용.
28. **★ InitializeSubUIs 호출 순서 주의** (2026-07-19): `GameRunState.StartRun()`이 `CurrentMap`/`CurrentStageTheme`을 생성하므로, InitializeSubUIs는 반드시 **StartRun 이후**에 호출. 이전 순서면 NodeDetailPanel/ThemeBanner Initialize가 null로 스킵됨.
29. **★ 서브 에이전트 병렬 활용 가치** (2026-07-19): 복잡한 Unity UI 버그 진단에서 `code-reviewer`/`code-explorer`/`general-purpose`(웹 검색) 에이전트가 90%+ 신뢰도로 핵심 원인 식별. AI 비전 도구(analyze_image 등)는 추측성이라 컨텍스트 풍부한 코드 리뷰 에이전트가 훨씬 정확.
30. **★ 밸런스 재조정 시 3개 소스 동기화** (2026-07-20): HP/위력 수치 변경 시 (a) CSV (`CharacterTable.csv`/`SkillTable.csv`) + (b) DataGenerator.PhaseCC.cs (플레이어 하드코딩) + (c) GameRunState.cs (엘리트 보너스 고정값) 3곳 모두 수정해야 일관성 유지. 하나라도 누락하면 밸런스 불일치.
31. **★ 비례 시스템(% 기반)은 자동 보정, 고정값은 수동 조정** (2026-07-20): 캠프파이어 힐 30%/부활 50%/어센션 PlayerMaxHp%/이벤트 HP% 등은 체력 축소 시 자동으로 따라감. 엘리트 보너스 +15 HP 같은 고정값은 비율 변화에 영향 안 받으므로 수동 조정 (×0.25 적용하여 +4로).
32. **★ 밸런스 패치 후 DataGenerator 메뉴 재실행 필수** (2026-07-20): CSV/코드를 수정해도 .asset 파일에 직렬화된 기존 값이 우선 (#25와 동일 패턴). `TeamLog/Generate Test Data` 메뉴 실행으로 모든 .asset 재생성 필요.
...
| COMBAT-EXP-2 | ★ 스킬 설명 UX 전면 개선 (2026-08-03) — 조건 시각화(자원/LimitBreak/CC/사용슬롯) + 슬롯/툴팁 분담. 슬롯은 수치+라벨("23 데미지 \| 2 화상"), 툴팁은 자연어 풀어쓰기("적에게 8의 피해를 줍니다. 잿빛 1스택마다...") + rich text 색상 강조(골드 숫자/주황 자원/이탤릭 조건). BehaviorTag 자연어 템플릿 30종 + Elara 자동 힐 3→1 밸런스 조정. 상세 `2026-08-03.md` |
...
- **★ Phase UI-2 (MapScene Rework) — 5차 세션 완료 (2026-07-19)**: 9 레이어 표준 구조 (Start + 4전투 + 보스 + 비전투 3) + 3-컬럼 다크 판타지 고딕 UI. 신규 12 UI 컴포넌트 + MapSceneSpriteGenerator(11 Sprite) + MapSceneReworkBuilder(main + Parts + Wiring) + Prefab 7종 (Node/Label×2/PlayerMarker/PartyMemberRow/RelicSlotCell/AugmentRow). **★ 5차에 걸친 재완성**: (1차) Builder에 Camera/Light/EventSystem 추가 + HeaderController + 비주얼 강화 + MapReworkDebugInitializer, (2차) Cinzel/Cormorant 고딕 폰트 + 색상 튜닝 + Header/Footer 비율 축소, (3차) ★ MainFrame VerticalLayoutGroup 제거 + anchor 직접 배치 (VLG가 LayoutElement 무시하는 Unity 한계 회피), (4차) 노드 anchor 수정 + 분기 배치 + 연결선 + containerHeight=0 폴백 + SerializeField spacing 강제 설정, (5차) Prefab 2종(RelicSlotCell/AugmentRow) + NodeDetailPanel 내부 자식 6종 + TestData 자동 로드 + ★ 치명 버그 3종 수정 (GetComponentInChildren 형제 못 찾음 / InitializeSubUIs 호출 순서 역전 / Refresh 폴밸 부재). **★ 핵심 인사이트**: (a) SerializeField 인스펙터 값이 코드 기본값보다 우선, (b) `GetComponentInChildren`는 자손만 검색 (형제는 `transform.parent.GetComponentInChildren` 또는 WireField), (c) 고정 크기 헤더/푸터는 anchor 직접 배치가 안전, (d) `containerHeight=0` 폴백 필수, (e) `Awake`에서 `enabled=false` 금지. **★ 서브 에이전트(code-reviewer/code-explorer/general-purpose) 병렬 활용이 90%+ 신뢰도 진단에 효과 입증**. 상세 `2026-07-19.md` (3-5차 세션 섹션). **잔여**: TitleScene→PartySelection→MapSceneRework 파이브 최종 런타임 검증 + Augments 섹션 CollectAllAugments TODO 해결 + 기존 MapScene 교체 결정
