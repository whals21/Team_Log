...
> **상태**: ✅ 구현 완료 (2026-08-03 기준 — Mercy 자원 코드 + 스킬 4종 .asset + 특성 3종 모두 구현)
> **슬롯**: Healer (기존 Char_Healer 리워크)
> **상위 문서**: [INDEX.md](INDEX.md), [CharacterConceptReview.md 5.6](../../CharacterConceptReview.md)
> **특성 에셋**: `Trait_Healer_Blessing/PureHeal/DivineShield.asset` (3종 모두 구현)
> **스킬 에셋**: `Elara_BondLink/MendWounds/BlessingOfMercy/Sanctuary.asset` (4종 모두 구현)
> **자원 코드**: `MercyResourceComponent.cs` (Phase CC-2C 구현 완료, 기획안과 100% 일치)
>
> **★ 2026-08-03 수치 갱신**: 2026-07-20 밸런스 축소로 인해 일부 스킬 위력/버프 값이 기획안에서 변경됨 (섹션 6 참조)
...
[매 턴 시작] → 연결된 파티원 각자에게 자동 힐 1 (★ 2026-08-03: 3→1 밸런스 조정)
              → Mercy +1 (모든 자동 힐이 Mercy 축전)
...
> **★ 2026-08-03 수치 갱신**: 게임 .asset 파일 기준으로 표 업데이트.
> 2026-07-20 밸런스 축소(위력 ×0.35)가 반영되어 기존 기획안에서 위력/버프 값이 변경됨.
> Mercy 자원 코드(`MercyResourceComponent.cs`)는 기획안과 100% 일치하게 구현됨.

| 스킬 | AP | 기본 효과 | 조건 | Mercy 효과 |
|-----|----|---------|------|-----------|
| **Bond Link** (연결의 끈) | 1 | 단일 연결고리 강화 — 대상은 **2턴 동안** 자동 힐 1 → 4으로 강화 (BondBoost 상태, +3) | 셋업 | (셋업 전용) |
| **Mend Wounds** (상처 치유) | 2 | 단일 즉시 힐 **3** + 도트 정화 (CleanseLowTarget) | 대상 부상 (HP≤50%) | Mercy +힐량 (MercyAccumulate) |
| **Blessing of Mercy** (자비의 축복) | 2 | 단일 **ATK+2** 버프 (3턴) | 자원 소모 (Mercy 5+) | Mercy -5 |
| **Sanctuary** (성소) | 3 | 광역 힐 **2** (전체 아군) | 자원 임계 (Mercy 8+) | Mercy -8, 각 파티원 회복량 추적 |

### 수치 변경 이력 (기획안 작성 이후)
| 스킬 | 기획안 (2026-07-17) | 게임 .asset (2026-08-03 확인) | 변경 이유 |
|------|---------------------|------------------------------|----------|
| Mend Wounds 힐 | 10 | **3** | 2026-07-20 밸런스 축소 (×0.35 → 3.5, 절사 3) |
| Sanctuary 힐 | 8 | **2** | 2026-07-20 밸런스 축소 (×0.35 → 2.8, 절사 2) |
| Blessing of Mercy ATK | +3 | **+2** | 2026-07-20 밸런스 축소 (버프 값 축소) |
| Bond Link 지속 | 1턴 | **2턴** | 밸런스 조정 (셋업 가치 상승) |
| Mend Wounds 조건 | HP<70% | HP≤50% | CleanseLowTarget 키워드 기준값 정렬 |
| Sanctuary 부가 | 연결고리 강화 명시 | BondBoost 부여 제외 (광역 힐만) | 단순화 |
...
| 특성 | 기존 효과 | 리워크 효과 | 해금 |
|------|---------|------------|------|
| **축복** (기본) | 힐 효과 +15% | **자동 힐 위력 +2 (1→3)** (연결고리 효율 증가) | 기본 |
...
> ⚠ **주의**: 아래 수치는 2026-07-17 기획안 기준 (Mend Wounds 10, Sanctuary 8 등).
> 2026-07-20 밸런스 축소 이후 실제 값은 섹션 6 참조. 시나리오는 흐름 이해용.
...
| 2026-08-03 | **★ 게임 .asset 수치로 갱신** — Mercy 코드 구현 완료 확인, Mend Wounds 10→3 / Sanctuary 8→2 / Blessing ATK+3→+2 / Bond Link 지속 1→2턴 반영. 상태 "구현 완료"로 승격 |
| 2026-08-03 (2차) | **★ BaseAutoHeal 3→1 밸런스 조정** — 자동 힐 과다로 인한 밸런스 완화. BondBoost 6→4, 축복 특성 5→3, 축복+BondBoost 8→6 |
